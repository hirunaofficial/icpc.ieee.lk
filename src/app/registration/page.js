      ...
      <FadeIn>
        <section className="relative py-32 overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
            <h1 className="text-6xl md:text-8xl font-bold mb-8 text-white">Registration</h1>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-20">
              ...
              <p className="text-2xl text-[#4C82C3] font-semibold">Follow these simple steps to register your team</p>
            </div>
          </FadeIn>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-20">
              ...
              <div className="h-2 w-24 bg-[#FDBA11] mx-auto mb-8 rounded-full"></div>
              <p className="text-2xl text-[#4C82C3] font-semibold">Everything you need to know before registering</p>
            </div>
          </FadeIn>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn>
            <div className="text-center mb-20">
              ...
              <div className="h-2 w-24 bg-[#FDBA11] mx-auto mb-8 rounded-full"></div>
              <p className="text-2xl text-[#4C82C3] font-semibold">Key details about the registration process</p>
            </div>
          </FadeIn>
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center">
              ...
              <ScaleIn delay={0.5}>
                <Link
                  href="/"
                  className="inline-block bg-white text-[#4C82C3] px-12 py-4 rounded-2xl text-xl font-bold hover:bg-gray-100 transition-all duration-300 hover:-translate-y-1 shadow-xl hover:shadow-2xl"
                >
                  Back to Home
                </Link>
              </ScaleIn>
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl p-10 hover:shadow-3xl transition-all duration-300 hover:-translate-y-2">
      <div className="flex items-start space-x-6 mb-8">
        <div className="flex-shrink-0 w-16 h-16 bg-[#4C82C3] text-white rounded-full flex items-center justify-center font-bold text-2xl shadow-xl">
          {number}
        </div>
        <div>
          <h3 className="text-3xl font-bold text-gray-900 mb-3">{title}</h3>
          <p className="text-lg text-[#4C82C3] font-semibold">{description}</p>
        </div>
      </div>
      ...
      <ul className="space-y-4 mb-8">
        {details && details.map((detail, index) => (
          <li key={`${title}-${index}`} className="flex items-start space-x-3 text-gray-700">
            <svg className="w-6 h-6 text-[#FDBA11] flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
      ...
      {buttonText && (
        external ? (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-block bg-[#4C82C3] text-white px-8 py-3 rounded-2xl font-bold text-lg hover:bg-[#3a6ba3] transition-all duration-300 hover:-translate-y-1 shadow-xl hover:shadow-2xl"
          >
            {buttonText} →
          </a>
        ) : (
...
function RequirementCard({ icon, title, items }) {
  return (
    <div className="bg-gray-50 border-2 border-transparent hover:border-[#4C82C3] rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-2">
      <div className="flex items-center space-x-4 mb-6">
        <div className="text-[#4C82C3] text-3xl">{icon}</div>
        <h3 className="text-2xl font-bold text-gray-900">{title}</h3>
      </div>
      <ul className="space-y-3">
        {items && items.map((item, index) => (
          <li key={`${title}-${index}`} className="flex items-start space-x-3 text-gray-700">
            <span className="text-[#FDBA11] font-bold text-xl">•</span>
...
function InfoItem({ icon, title, content }) {
  return (
    <div className="flex items-start space-x-6 pb-8 border-b-2 border-gray-200 last:border-0">
      <div className="text-[#4C82C3] flex-shrink-0 text-2xl">{icon}</div>
      <div>
