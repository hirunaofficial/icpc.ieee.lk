    ...
    <main className="min-h-screen bg-white">
      ...
      <section className="py-24 bg-gray-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="relative">
            {/* Vertical Line */}
            <div className="absolute left-8 top-0 bottom-0 w-2 bg-[#4C82C3] rounded-full"></div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <ScaleIn delay={0.2}>
            <div className="bg-[#FDBA11] rounded-3xl p-12 shadow-2xl">
...
function TimelineEvent({ date, title, description, icon, color, highlight, actions }) {
  return (
    <div className="relative pl-24">
      ...
      {/* Content */}
      <div className={`${highlight ? 'bg-[#FDBA11]' : 'bg-white'} rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-300 hover:-translate-y-2 p-10`}>
        <div className={`text-lg font-bold ${highlight ? 'text-[#4C82C3]' : 'text-gray-600'} mb-4 uppercase tracking-wide`}>{date}</div>
        <h3 className={`text-3xl font-bold ${highlight ? 'text-[#4C82C3]' : 'text-gray-900'} mb-6`}>{title}</h3>
        <p className={`${highlight ? 'text-[#4C82C3]' : 'text-gray-700'} mb-8 text-lg leading-relaxed`}>{description}</p>
        
        {actions && actions.length > 0 && (
          <ul className="space-y-4">
            {actions.map((action, actionIndex) => (
              <li key={`${title}-${actionIndex}`} className={`flex items-start space-x-4 ${highlight ? 'text-[#4C82C3]' : 'text-gray-700'}`}>
                <svg className="w-6 h-6 text-[#FDBA11] flex-shrink-0 mt-1" fill="currentColor" viewBox="0 0 20 20">
...
function QuickDate({ date, event, highlight }) {
  return (
    <div className={`flex items-center justify-between p-6 ${highlight ? 'bg-white border-l-8 border-[#B22E1B]' : 'bg-[#4C82C3]/10'} rounded-2xl hover:shadow-lg transition-all duration-300`}>
      <span className={`font-bold text-xl ${highlight ? 'text-[#B22E1B]' : 'text-[#4C82C3]'}`}>{date}</span>
      <span className={`text-xl ${highlight ? 'text-gray-900' : 'text-[#4C82C3]'} font-semibold`}>{event}</span>
    </div>
  );
}
